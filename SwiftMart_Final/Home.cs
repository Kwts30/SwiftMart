...
{
    public partial class LoadAdminHome : Form
    {
        ...
        public LoadAdminHome()
        {
            InitializeComponent();
            paymentComboBox.SelectedIndexChanged += cmbPaymentMethod_SelectedIndexChanged;
            ...
            btncanned1.Tag = "Dole 100% Pineapple Juice \r\nDrink Unsweetened 240ml";
            btncanned1.Click += ItemButton_Click;

            btncanned2.Tag = "Young's Town Sardines \r\nIn Tomato Sauce 155g";
            btncanned2.Click += ItemButton_Click;

            btncanned3.Tag = "Young's Town Sardines In \r\nTomato Sauce With \r\nHot Chili 155g";
            btncanned3.Click += ItemButton_Click;

            btncanned4.Tag = " Argentina Corned Beef 150g";
            btncanned4.Click += ItemButton_Click;

            btncanned5.Tag = "Mega Sardines In Tomato \r\nSauce 155g";
            btncanned5.Click += ItemButton_Click;

            //frozen

            btnfrozen1.Tag = "Purefoods Chicken Breast \r\nNuggets 1kg";
            btnfrozen1.Click += ItemButton_Click;

            btnfrozen2.Tag = "Sarangani Bay Boneless \r\nMilkfish Unseasoned Medium 320g";
            btnfrozen2.Click += ItemButton_Click;

            btnfrozen3.Tag = "Purefoods Chicken Breast\r\nNuggets 200g";
            btnfrozen3.Click += ItemButton_Click;

            btnfrozen4.Tag = "Virginia Premium Hotdog \r\nRegular 1kg";
            btnfrozen4.Click += ItemButton_Click;

            btnfrozen5.Tag = "Purefoods Beef Tapa 220g";
            btnfrozen5.Click += ItemButton_Click;

            //beverages

            btnbeverages1.Tag = "Kopiko Brown Coffee Twin \r\nPack 53g";
            btnbeverages1.Click += ItemButton_Click;

            btnbeverages2.Tag = "Coke Regular 1L";
            btnbeverages2.Click += ItemButton_Click;

            btnbeverages3.Tag = "Nature's Spring Distilled \r\nWater 10L";
            btnbeverages3.Click += ItemButton_Click;

            btnbeverages4.Tag = "Kopiko Blanca Creamy \r\nCoffee Twin Pack\r\n 58g";
            btnbeverages4.Click += ItemButton_Click;

            btnbeverages5.Tag = "Sprite Regular 1L";
            btnbeverages5.Click += ItemButton_Click;
        ...
        private void cmbPaymentMethod_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // Update lblPaymentMethod based on selected payment method
        ...
        {
            // Check if the DataGridView has items
            if (dataGridViewItems.Rows.Count == 0)
            {
                MessageBox.Show("Please select items before proceeding to payment.");
                return;
            }

            // Ensure payment and delivery methods are selected
            ...
            {
                MessageBox.Show("Please select a delivery method.");
                return;
            }
            ...
            // Update database for each item in the cart
            UpdateItemQuantitiesInDatabase();

            // Optionally, clear fields after successful payment
            ClearFields();
        }

        private void UpdateItemQuantitiesInDatabase()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    foreach (DataGridViewRow row in dataGridViewItems.Rows)
                    {
                        if (row.Cells["columnItemName"].Value != null && row.Cells["columnItemQuantity"].Value != null)
                        {
                            string itemName = row.Cells["columnItemName"].Value.ToString();
                            int quantityPurchased = Convert.ToInt32(row.Cells["columnItemQuantity"].Value);

                            // Update item stocks and item_sold count in the database
                            using (SqlCommand command = new SqlCommand(
                                "UPDATE Items SET item_stocks = item_stocks - @QuantityPurchased, item_sold = ISNULL(item_sold, 0) + @QuantityPurchased WHERE item_name = @ItemName",
                                connection))
                            {
                                command.Parameters.AddWithValue("@QuantityPurchased", quantityPurchased);
                                command.Parameters.AddWithValue("@ItemName", itemName);

                                int rowsAffected = command.ExecuteNonQuery();
                                if (rowsAffected == 0)
                                {
                                    MessageBox.Show($"Failed to update item '{itemName}' in the database.");
                                }
                            }
                        }
                    }
            ...
            {
                MessageBox.Show("Error updating item quantities: " + ex.Message);
            }
        }
